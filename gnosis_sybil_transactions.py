...
groups_df = pd.read_csv(r"C:\gnosis_large_connected_groups.csv")
        ...
        for tx in transactions:
            ...
            if tx["from"] in addresses and tx["to"] in addresses and tx["from"] != tx["to"] and tx["input"] == "0x":
                transfer_str = (f'hash: "{tx["hash"]}" from: "{tx["from"]}" to: "{tx["to"]}" '
                                f'tokenSymbol: "DAI" value: "{int(tx["value"]) / (10 ** 18)}"')
...
# save
output_file_path = r"C:\gnosis_token_transfers_per_group.csv"
