...
groups_df = pd.read_csv(r"C:\moonbeam_large_connected_groups.csv")
        ...
        for tx in transactions:
            ...
            if tx["from"] in addresses and tx["to"] in addresses and tx["from"] != tx["to"]:
                transfer_str = (f'hash: "{tx["hash"]}" from: "{tx["from"]}" to: "{tx["to"]}" '
                                f'tokenSymbol: "GLMR" value: "{int(tx["value"]) / (10 ** 18)}"')
...
# save
output_file_path = r"C:\moonbeam_token_transfers_per_group.csv"
